...
private const val CAM_HEIGHT   = 22f   // lifted slightly for the bigger boss
...
// Sword hit: player must be within this many world-units of the boss node.
// Boss is ~2.4 units tall, model centre is offset, so 4 units lets the player
// reach the torso without requiring pixel-perfect positioning.
...
private data class FloatNum(
    val text:      BitmapText,
    val color:     ColorRGBA,   // base color (alpha mutated each frame)
    var screenX:   Float,
    var screenY:   Float,
    var life:      Float,
...
class DungeonGame : SimpleApplication() {
    ...
    // World-space Y at which the boss was spawned — used to keep it grounded
    // after animations that try to drift the root joint vertically.
    ...
    /** Called (on GL thread) whenever the game ends.  Wire up from MainActivity. */
    ...
    private var lastPlayerHp  = -1     // cached to skip redundant setText
    private var lastBossHp    = -1

    // Floating damage numbers
    private val floatNums = mutableListOf<FloatNum>()

    // Shared input (written by Android UI / desktop)
    ...
    private fun setupGround() {
        groundGeom = Geometry("Ground", Quad(40f, 40f)).apply {
        ...
        groundGeom.addControl(RigidBodyControl(0f).also { bulletState.physicsSpace.add(it) })
    }

    private fun setupWalls() {
        // Expanded arena: 28×28 world-units (walls at ±14)
        val defs = listOf(
        ...
        for ((cx, cy, cz, hw, hh, hd) in defs) {
            val g = Geometry("Wall", Box(hw, hh, hd)).apply {
                material = wallMat
                setLocalTranslation(cx, cy, cz)
            }
            rootNode.attachChild(g)
            wallGeoms += g
            g.addControl(RigidBodyControl(0f).also { bulletState.physicsSpace.add(it) })
        }
    }
    ...
    fun restartGame() {
        ...
        input.swordQueued = false;  input.dashQueued = false;  input.shieldHeld = false
    ...
    private fun movePlayer(tpf: Float) {
        ...
        when {
            ...
            targetPosition != null -> {
                val toT = targetPosition!!.subtract(playerNode.localTranslation).also { it.y = 0f }
                if (toT.length() < 0.08f) { targetPosition = null; lastMoveDistance = 0f; return }
    ...
    private fun updateBoss(tpf: Float) {
        ...
        // Pin boss Y to ground — some animation root-bone channels can drift it upward.
        ...
        if (bossHit && !input.shieldHeld) {
            ...
            if (wasAlive && playerHp == 0) {
                ...
                spawnDmgNum(playerNode.localTranslation.add(0f, 1.5f, 0f), "-1",
                    ColorRGBA(1f, 0.25f, 0.25f, 1f))
            }
        }
    ...
    private fun checkSwordHit() {
        ...
        // Active frames: first 60 % of the swing arc
        if ((1f - attackTimer / ATTACK_DURATION) > 0.60f) return
        ...
        if (dist <= SWORD_HIT_RANGE) {
            ...
            spawnDmgNum(dragonBoss.node.localTranslation.add(0f, 2.5f, 0f), "-1",
                ColorRGBA(1f, 0.85f, 0.10f, 1f))
        }
    }
    ...
    private fun endGame(won: Boolean) {
        ...
        restartHint.text  = "Tap / press Play Again to restart"
    ...
    private fun updateHUD() {
        val bossRatio   = dragonBoss.hp.toFloat() / dragonBoss.maxHp
        val playerRatio = playerHp.toFloat() / PLAYER_MAX_HP

        bossHpFill.setLocalScale(bossRatio.coerceIn(0f, 1f), 1f, 1f)
        playerHpFill.setLocalScale(playerRatio.coerceIn(0f, 1f), 1f, 1f)

        // Only call setText when the value actually changed (avoid per-frame string alloc).
    ...
    private fun tickFloatNums(tpf: Float) {
        val iter = floatNums.iterator()
        while (iter.hasNext()) {
            val fn = iter.next()
            fn.life -= tpf
            if (fn.life <= 0f) {
                guiNode.detachChild(fn.text)
                iter.remove()
                continue
            }
            val progress = 1f - fn.life / fn.maxLife   // 0 → 1
            val newY = fn.screenY + progress * 55f
            fn.text.setLocalTranslation(fn.screenX, newY, 5f)
